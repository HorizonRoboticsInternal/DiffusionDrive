...
class FieryModel(nn.Module):
    ...
    def __init__(self, config: FieryConfig):
        ...
        self._config = config
        self._backbone = Fiery(config.encoder_cfg, pretrained=config.encoder_pretrained, freeze_encoder=config.encoder_freeze)
        self._bev_encoder = E2EBEVEncoder(config.raster_num_input_channels, config.bev_embed_dims, freeze=config.bev_encoder_freeze)

        self._keyval_embedding = nn.Embedding(8 * 16 + 1, config.tf_d_model)  # 8x8 feature grid + trajectory
        ...
        self._bev_semantic_head = nn.Sequential(
            ...
            nn.Upsample(
                size=(config.lidar_resolution_height, config.lidar_resolution_width),
                mode="bilinear",
                align_corners=False,
            ),
        )
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None) -> Dict[str, torch.Tensor]:
        ...
        extrinsics = features['extrinsics']
        future_egomotion = features['future_egomotion']
        status_feature: torch.Tensor = features["status_feature"]

        batch_size = status_feature.shape[0]

        bev_feature_upscale, _ = self._backbone(
            image=cameras,
...
class TrajectoryHead(nn.Module):
    ...
    def __init__(self, num_poses: int, d_ffn: int, d_model: int, plan_anchor_path: str,config: FieryConfig):
        """
    ...
    def forward_test(self, ego_query,agents_query,bev_feature,bev_spatial_shape,status_encoding,global_img) -> Dict[str, torch.Tensor]:
        ...
        bs = ego_query.shape[0]
        device = ego_query.device
        self.diffusion_scheduler.set_timesteps(100, device)
        ...
        trunc_timesteps = torch.full((bs,), roll_timesteps[0].item(), dtype=torch.long, device=device)
