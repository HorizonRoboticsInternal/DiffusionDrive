...
class EMA(Callback):
    ...
    def _maybe_update(self, global_step, pl_module):
        update_fn = getattr(pl_module.agent._model._trajectory_head, "update_reference_ema", None)
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if (trainer.global_step + 1) % self.every_n_steps == 0:
            self._maybe_update(trainer.global_step, pl_module)

    # def on_validation_end(self, trainer, pl_module):
    #     # optional: refresh EMA at validation end as well
    #     self._maybe_update(pl_module)
